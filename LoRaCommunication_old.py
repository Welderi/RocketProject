...
def Byte2Bytearray(byteArr:bytearray, byteNum:int=0, byteVal:int=0):
    if(byteArr == bytearray) & byteNum <= len(bytearray):
...
class LoRaInstance:
    ...
    async def SendData(self, packet: bytearray):
    ...
    async def GetReceivedData(self):
        if self._ValidateReceivedData():
            return self._buffer
        return None


class MessageToSendFormater:   # Тут може буду утєчка памяті. POZOR!!!(Підчищати після отправки)
    # Якшо відправляїця меседж типу "status" data:INT треба поскладати отдєльно, ППЕРЕД ВИЗОВОМ ОБЬЄКТА
    def __init__(self, messageType: MessageToSendType, dataToSend: int):
        ...
        #if self._MessageIsValid():
        self._MessageFormating()

    """def _MessageIsValid(self):
        if self._messageType == MessageToSendType.status:
            t = type(self._data)
            if (t == StatusToSendType) | (t == AlarmToSendType) | (t == StateToSendType):
                return True
            else: return False
...
class ReceivedMessageHandler:  # Тут може буду утєчка памяті. POZOR!!!(Підчищати після прийома)
    def __init__(self, rawMessage: bytearray):
        self._raw = rawMessage
        # self._returnDict = {"command": , "liveByte": self._raw[2]}
